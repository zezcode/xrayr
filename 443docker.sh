...
[[ $EUID -ne 0 ]] && echo -e "[${red}Error${plain}] Vui lòng chạy với quyền Root!" && exit 1
...
pre_install_docker_compose() {
    read -p "Nhập Link Web (VD: https://example.com): " domain
    echo -e "Link Web : ${domain}"

    read -p "Nhập Api Key(Khoá giao tiếp của web): " APIKEY
    echo -e "API KEY : ${APIKEY}"

    read -p "Nhập Node ID port 443 : " node_443
    echo -e "Node 443 : ${node_443}"

    read -p "Nhập Subdomain hoặc IP VPS (Khác Subdomain Port 80): " CertDomain443
    echo -e "CertDomain Port 443 = ${CertDomain443}"

    read -p "Nhập Mail Cloudfalre: " mailcf
    echo -e "Mail Cloudflare : ${mailcf}"

    read -p "Nhập API TOKEN Cloudflare: " token
    echo -e "API TOKEN Cloudflare : ${token}"
}
...
services: 
  xrayr: 
    ...
    volumes:
      - ./config.yml:/etc/XrayR/config.yml # Thư mục cài file Config
...
chmod +x /usr/local/bin/docker-compose
  ...
  echo -e "Cài đặt tự khởi chạy đã hoàn tất! Hệ thống sẽ Update sau [${green}24H${plain}] Từ lúc bạn cài đặt"
}
...
install_dependencies() {
  if check_sys packageManager yum; then
    ...
    echo -e "[${green}Info${plain}] Đang kiểm tra kho lưu trữ EPEL..."
...
clear
while true; do
  echo "Phiên bản này đã được tối ưu hóa từ XrayR Project"
  echo "Đã được fix lỗi Zalo và một số lỗi vặt khác"
