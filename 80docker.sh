...
pre_install_docker_compose() {
    ...
    read -p "Nhập Api Key (Khóa giao tiếp của web): " APIKEY
    echo -e "API KEY : ${APIKEY}"

    read -p "Nhập Node ID Port 80 :" node_80
    echo -e "Node 80 : ${node_80}"

    read -p "Nhập Subdomain hoặc IP VPS (Khác Subdomain Port 443): " CertDomain80
    echo -e "CertDomain Port 80 = ${CertDomain80}"
}
    ...
    ControllerConfig:
      ...
      CertConfig:
        ...
        DNSEnv: # DNS ENV option used by DNS provider
          CLOUDFLARE_EMAIL: ht4gvpn
          CLOUDFLARE_API_KEY: ht4gvpn
EOF
