...
install_XrayR() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://api.github.com/repos/overkillzero/xrayr/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        url="https://github.com/overkillzero/xrayr/releases/download/${last_version}/XrayR-linux-${arch}.zip"
    ...
    curl -o /usr/bin/XrayR -Ls https://raw.githubusercontent.com/overkillzero/xrayr/main/xrayz.sh
